...
- Use double quotes around echo commands so $(date) is evaluated.
- Adds separate stderr redirection to an .err file.
- Adds echo timestamps (start and end) into the .out log file.
...
def main():
    ...
    for ligand in sorted(os.listdir(indir)):
        ...
        cmd = (
            ...
            f"vina --config \"{cfg}\" --ligand \"{lig_path}\" --out \"{out_path}\" --cpu 1 --spacing 0.1 >> \"{out_redirect}\" 2>\"{err_redirect}\" && "
