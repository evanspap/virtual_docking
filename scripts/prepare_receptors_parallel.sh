...
#   Uses GNU Parallel to process multiple files at once.
#
# Usage:
...
process_one() {
    ...
    reduce_out="${indir}/${base}_reduce.pdb"    # e.g. ./pdb/ADSS2_b_filter_reduce.pdb
...
export indir outdir dry_run

# Run in parallel using 28 cores (or detect via SLURM_CPUS_PER_TASK)
ls "$indir"/*_b_filter.pdb | parallel -j ${SLURM_CPUS_PER_TASK:-28} process_one {}
